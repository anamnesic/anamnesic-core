...
describe("kairos SDK package e2e", () => {
  ...
  it("packs and imports from an external temp consumer", async () => {
    ...
    const packageRoot = path.join(repoRoot, "terminal", "packages", "sdk");
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "kairos-sdk-consumer-"));
    tempDirs.push(tempDir);

    await runCommand("pnpm", ["--filter", "@kairos-ai/sdk", "build"], {
      cwd: repoRoot,
      timeoutMs: 180_000,
    });
    ...
    const importScript = `
      import { GatewayClientTransport, kairos, normalizeGatewayEvent } from "@kairos-ai/sdk";
